...
marketplace module (or Internal Catalog). This sample fully automates the data sharing
process between two pre-existing Data Share accounts.
...
- Sets up a dataset from a ADLS Gen2 storage account file system
...
- Creates mappings for the shared datasets that point to an ADLS Gen2 storage
  account file system
...
- `dest.py` will execute with the service principal identity so it can accept
...
After the script executes you should see a subscription setup on the Received
...
> Note: the function will quickly exit if no invitations are found. You can use
